        ...
        <div className="min-h-screen bg-gray-900 p-8">
            ...
            {applications.length === 0 ? (
                ...
                applications.map((application) => (
                    <motion.div
                        key={application._id}
                        className="mb-10 bg-gray-800 rounded-xl shadow-xl p-8"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                    >
                        <h2 className="text-3xl text-indigo-400 font-bold mb-4">{application.jobId.name}</h2>
                        ...
                        {application.status === 'Accepted' && (
                            <p className="text-green-400 font-bold mb-4">Congratulations! You have been selected for this position.</p>
                        )}
                        {application.status === 'Rejected' && (
                            <p className="text-red-400 font-bold mb-4">We regret to inform you that you were not selected for this position. Thank you for applying!</p>
                        )}
                        {application.status === 'Shortlisted' && (
                            <p className="text-yellow-400 font-bold mb-4">Great news! You have been shortlisted. We will contact you soon with further details.</p>
                        )}
                        {application.status === 'Applied' && (
                            <p className="text-blue-400 font-bold mb-4">Your application has been submitted successfully. We will review it and get back to you soon.</p>
                        )}
